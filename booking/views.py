...
def room_update(request, room_id):
	...
	if request.method == "POST":
		...
		if Meeting.objects.filter(pk=_id).count()==0:
			start_dt = get_pakistan_dt(start)
			end_dt = get_pakistan_dt(end) # + timedelta(hours=1)
			print(start_dt)
			print(end_dt)
...
def get_pakistan_dt(ts):
	tz = pytz.timezone('Asia/Karachi')
	return datetime.fromtimestamp(int(ts[0:10]), tz)


def check_valid_duration(request, start, end, id):
	ds_now = datetime.now()
	tz = pytz.timezone('Asia/Karachi')
	ds_now = ds_now.astimezone(tz)
	...
	if Meeting.objects.filter(end__gte=start, start__lt=end, start__gte=start).exclude(id=id).count() > 0:
		...
		return False
	elif start <=  ds_now - timedelta(minutes=5):
